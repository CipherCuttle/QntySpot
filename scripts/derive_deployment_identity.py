...
PRE_INK_V0F_NATIVE_SELL_SOURCE_PATHS = tuple(
    path
    for path in SOURCE_PATHS
    if path != "qntyspot/ink_v0f_native_sell.py"
)
PRE_INK_V0F_NATIVE_SOURCE_PATHS = tuple(
    path
    for path in PRE_INK_V0F_NATIVE_SELL_SOURCE_PATHS
...
PRE_INK_V0F_HUMAN_SIGNING_SOURCE_PATHS = tuple(
    path
    for path in PRE_INK_V0F_SIGNED_SWAP_SOURCE_PATHS
...
PRE_INK_V0F_HANDOFF_SOURCE_PATHS = tuple(
    path
    for path in PRE_INK_V0F_PREAUTH_SOURCE_PATHS
    if path
    not in {
        "qntyspot/ink_v0f_execution.py",
        "artifacts/ink_v0f/INK_V0F_ROUTER_IDENTITY_V0.json",
...
def _source_manifest(root: Path) -> list[dict[str, str]]:
    ...
    elif not (root / "qntyspot/ink_v0f_preauth.py").exists():
        # Preserve the pre-router-preauth runtime identity while binding the
    ...
    elif not (root / "qntyspot/ink_v0f_signed_swap.py").exists():
        # Preserve the human-signing-settlement runtime identity while binding
    ...
    elif not (root / "qntyspot/ink_v0f_native_sell.py").exists():
        # Preserve the native-BUY-only identity while binding native ETH
        # settlement for SELL into current deployment identity.
        manifest_paths = PRE_INK_V0F_NATIVE_SELL_SOURCE_PATHS
    else:
