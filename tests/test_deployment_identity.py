...
def test_current_identity_includes_dynamic_accepted_intent_consumer() -> None:
...
def test_current_identity_includes_authenticated_policy_bridge() -> None:
    identity = build_identity(ROOT, CANONICAL_COMMIT)
    paths = [item["path"] for item in identity["implementation_identity"]["file_manifest"]]
...
def test_current_identity_includes_prelive_capital_economics() -> None:
    identity = build_identity(ROOT, CANONICAL_COMMIT)
    paths = [item["path"] for item in identity["implementation_identity"]["file_manifest"]]
    assert "qntyspot/prelive_economics.py" in paths


def test_current_identity_includes_ink_v0f_risk_consumer_and_artifact() -> None:
    identity = build_identity(ROOT, CANONICAL_COMMIT)
    paths = [item["path"] for item in identity["implementation_identity"]["file_manifest"]]
...
def test_current_identity_includes_ink_v0f_router_preauth() -> None:
    identity = build_identity(ROOT, CANONICAL_COMMIT)
    paths = [item["path"] for item in identity["implementation_identity"]["file_manifest"]]
...
def test_current_identity_includes_ink_v0f_signed_swap_rehearsal() -> None:
    identity = build_identity(ROOT, CANONICAL_COMMIT)
    paths = [item["path"] for item in identity["implementation_identity"]["file_manifest"]]
